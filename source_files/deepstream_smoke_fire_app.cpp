...
namespace SmokeFireDet {
  void
  Fire::update_fps(gint id) {
  ...
  int
  Fire::create_input_sources(gpointer pipe, gpointer mux, guint num_sources) {
  ...
  void
  Fire::changeBBoxColor(gpointer obj_meta_data, int has_bg_color, float red, float green,
  ...
  void
  Fire::addDisplayMeta(gpointer batch_meta_data, gpointer frame_meta_data) {
  ...
  GstPadProbeReturn
  Fire::tiler_src_pad_buffer_probe(GstPad *pad, GstPadProbeInfo *info,
                            gpointer u_data) {
...
if(class_index == FIRE) {
  ...
  gboolean
  Fire::bus_call(GstBus *bus, GstMessage *msg, gpointer data) {
  ...
  void
  Fire::cb_newpad(GstElement *decodebin, GstPad *decoder_src_pad, gpointer data) {
  ...
  void
  Fire::decodebin_child_added(GstChildProxy *child_proxy, GObject *object, gchar *name, gpointer user_data) {
  ...
  GstElement *
  Fire::create_source_bin(guint index, gchar *uri) {
  ...
  gchar *
  Fire::get_absolute_file_path(gchar *cfg_file_path, gchar *file_path) {
  ...
  gboolean
  Fire::set_tracker_properties(GstElement *nvtracker) {
    gboolean ret = FALSE;
    GError *error = NULL;
    gchar **keys = NULL;
    gchar **key = NULL;
  ...
  int
  Fire::configure_element_properties(int num_sources, GstElement *streammux, GstElement *pgie_yolo_detector,
    ...
    if (boost::filesystem::exists(boost::filesystem::path(
        SmokeFireDet::Fire::PGIE_YOLO_ENGINE_PATH))) {

      g_object_set(G_OBJECT(pgie_yolo_detector),
                  "model-engine-file", SmokeFireDet::Fire::PGIE_YOLO_ENGINE_PATH.c_str(), NULL);
    }
    else {
    ...
    if (!SmokeFireDet::Fire::set_tracker_properties(nvtracker)) {
  ...
  void Fire::setPaths(guint num_sources) {

    // Config Paths
    PGIE_YOLO_DETECTOR_CONFIG_FILE_PATH =
    strdup("models/fire/config_infer_primary_yolov5m.txt");
...
int main(int argc, char *argv[]) {
  ...
  ctx = g_option_context_new("Fire DeepStream App");
  group = g_option_group_new("Fire", NULL, NULL, NULL, NULL);
  g_option_group_add_entries(group, Fire.entries);
  ...
  gint sources = Fire.create_input_sources(pipeline, streammux, num_sources);
  if (sources == -1) {
    return -1;
  }
  else {
  ...
  int fail_safe = Fire.configure_element_properties(num_sources, streammux, pgie_yolo_detector,
  ...
  bus_watch_id = gst_bus_add_watch(bus, Fire.bus_call, loop);
  ...
  if (!tiler_src_pad) {
    ...
    gst_pad_add_probe(tiler_src_pad, GST_PAD_PROBE_TYPE_BUFFER,
                      Fire.tiler_src_pad_buffer_probe, NULL, NULL);
  }
