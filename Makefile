...
SRCS+= $(wildcard source_files/*.cpp)

INCS:= $(wildcard source_files/*.h)
...
yolov3:
	...
	cd custom_parsers/nvds_customparser_yolov5 && $(MAKE)

clean:
	rm -rf $(OBJS) $(APP)
	#cd custom_parsers/nvds_customparser_yolov3 && $(MAKE) clean
	cd custom_parsers/nvds_customparser_yolov5 && $(MAKE) clean
